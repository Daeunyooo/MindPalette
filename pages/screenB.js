    ...
    <>
      ...
      <div className="flex items-center h-screen flex-col md:pt-2 pt-2">
        ...
        <div style={{ width: "900px" }}>
          <div className="flex justify-start mt-4 flex-col text-left text-lg">
            <p className="m-1">
              <strong>AI:</strong> What has made you happy recently? Feel free
              to draw it or respond.
            </p>
            {conversationHistory.map((turn, index) => (
              <div key={index}>
                <p className="m-1">
                  <strong className="text-blue-500">You:</strong> {turn.user}
                </p>
                <p className="m-1">
                  <strong className="text-green-700">AI:</strong> {turn.ai}
                </p>
              </div>
            ))}
             <div>{isLoading && <LoadingDots /> }</div>
          </div>
          <p className="text-lg">Transcript:</p>
          <div className="flex flex-row justify-between border-2 border-black">
            <p className="text-lg">{transcript}</p>

            {notSpeaking && !doneSpeaking && (
              <button
                className="text-lg border-l-2 p-1"
